...
def viter(pinyin_list, alpha = 0.9, beta = 0.97):
    '''
    ...
    for i in range(1, len(pinyin_list)):
        ...
        emi_res = dict(cur.execute('''SELECT EMISSION.WORD, EMISSION.PROBABILITY
                                    ...
                                    WHERE EMISSION.PINYIN = "{}"
                                    '''.format(py)))
        tran_res = cur.execute('''SELECT TRANSMISSION.PRE, TRANSMISSION.REAR, TRANSMISSION.PROBABILITY
                        FROM TRANSMISSION
                        JOIN EMISSION
        ...
        for rear in candidates:
            ...
            for pre in word_path.keys():
                if pre in tran_dict.keys():
                    prob = -log10(beta * (10 ** (-tran_dict[pre])) + \
                            (1 - beta) * (10 ** -(word_p[rear]))) + \
                            dp[(i-1)%2][pre]
                else:
                    prob = -log10(1 - beta) + word_p[rear] + dp[(i-1)%2][pre]
                if prob < best_p:
                    best_p = prob
                    best_pre = pre
